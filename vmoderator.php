...
function flagged() {
	global $post, $vms; //Displays the unflag option
	$flags = get_post_meta($post->ID, '_flags', TRUE);
	$key = crc32(wp_get_current_user()->data->user_pass);
...
function vmoderator($classes = NULL) { //The function to display flag arya.
	...
	if (is_user_logged_in()) {
		$culevel = get_userdata($user)->user_level;
	} else {
		$culevel = - 1;
	};
	if (is_array($classes)) {
	...
	if (in_array($post->post_author, explode(',', str_replace(' ', '', $vms['safeUsers'])))) {
		$die = TRUE;
	}
	if ($vms['noFlagAdmin'] && $alevel > 7) {
		$die = TRUE;
	}
	if ($vms['noFlagEditor'] && $alevel > 2 && $alevel < 8) {
		$die = TRUE;
	}
	...
	if (!$vms['flags2removePost'] > 0) {
		$die = TRUE;
	}
	if ($vms['canFlag'] > $culevel) {
		$die = TRUE;
	}
	if (!$die) {
		return "<div id='flag-arya-" . $post->ID . "' class='" . $classes . " flag-arya flag-arya-" . $post->ID . "'>" . flag_arya() . "</div>";
	};
